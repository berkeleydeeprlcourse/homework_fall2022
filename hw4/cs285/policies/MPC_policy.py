...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        candidate_action_sequences = rng.uniform(self.low, self.high, (num_sequences, horizon, self.ac_dim))
        ...
        if self.sample_strategy == 'random' or (self.sample_strategy == 'cem' and obs is None):
            ...
            mu = np.mean(candidate_action_sequences, axis=0)
            ...
            for i in range(self.cem_iterations):
                ...
                #a_elites_ind = np.argpartition(predicted_rewards, -self.cem_num_elites)[-self.cem_num_elites:]
                #a_elites = candidate_action_sequences[a_elites_ind, :, :]

                a_elites = candidate_action_sequences[np.argsort(predicted_rewards)[-self.cem_num_elites:], :]

                mu = self.cem_alpha * np.mean(a_elites, axis=0) + (1 - self.cem_alpha) * mu
                var = self.cem_alpha * np.var(a_elites, axis=0) + (1 - self.cem_alpha) * var
