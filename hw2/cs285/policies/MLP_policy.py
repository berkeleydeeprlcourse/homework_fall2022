...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        # ones = ptu.from_numpy(np.ones_like(advantages))
        ...
        if self.nn_baseline:
            ...
            q_val_mean = np.mean(q_values)
            q_val_std = np.std(q_values)
            q_values = (q_values - q_val_mean) / q_val_std

            q_values = ptu.from_numpy(q_values)
            baseline_loss = self.baseline_loss(self.baseline(observations), q_values)
